...
public class SkeletonSpawnerBlockEntity extends BlockEntity {

    private final MobSpawnerLogic logic = new MobSpawnerLogic() {
        public void sendStatus(World world, BlockPos pos, int i) {
            world.addSyncedBlockEvent(pos, Main.SKELETON_SPAWNER, i, 0);
        }
        ...
        public void setSpawnEntry(@Nullable World world, BlockPos pos, MobSpawnerEntry spawnEntry) {
            super.setSpawnEntry(world, pos, new MobSpawnerEntry((NbtCompound) Util.make(new NbtCompound(), (nbt) -> {
                nbt.putString("id", "minecraft:skeleton");
            }), Optional.empty()));
            if (world != null) {
    ...
    public SkeletonSpawnerBlockEntity(BlockPos pos, BlockState state) {
        super(Main.SKELETON_SPAWNER_BE, pos, state);
    ...
    public static void clientTick(World world, BlockPos pos, BlockState state, @NotNull SkeletonSpawnerBlockEntity blockEntity) {
        blockEntity.logic.clientTick(world, pos);
    }

    public static void serverTick(World world, BlockPos pos, BlockState state, @NotNull SkeletonSpawnerBlockEntity blockEntity) {
